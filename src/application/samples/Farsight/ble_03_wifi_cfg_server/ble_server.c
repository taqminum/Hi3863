                                             ...
                                             errcode_t status)
{
    ...
    int result = parseWifiInfo((const char*)write_cb_para->value, &my_wifi_info) ;
    // 处理解析结果
    if (result == 0) {
        my_wifi_info.wifi_flag[0]=1;
    }
    else 
    {
        printf ("解析失败，错误代码: % d\n", result);
        switch (result) 
        {
            case -1: printf ("输入参数无效 \n"); break;
            case -2: printf ("字符串格式错误，应使用 [和] 包裹 \n"); break;
            case -3: printf ("未找到 SSID 字段 \n"); break;
            case -4: printf ("SSID 格式错误，缺少闭合引号 \n"); break;
            case -5: printf ("SSID 过长 \n"); break;
            case -6: printf ("未找到 PWD 字段 \n"); break;
            case -7: printf ("PWD 格式错误，缺少闭合引号 \n"); break;
            case -8: printf ("PWD 过长 \n"); break;
            default: printf ("未知错误 \n");
        }
    }
}
