...
extern char IP[20];
extern wifi_linked_info_stru wifi_status;
...
void oled_show(void)
{
    ...
    if(wifi_status.conn_state == WIFI_CONNECTED){
         ...
         SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT8_LINE_0, "wifi connected", TEXT_SIZE_8);
         if (sprintf((char *)oledShowBuff, "ip: %s", IP) > 0) 
             SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT8_LINE_1, oledShowBuff, TEXT_SIZE_8);
    }else
    {
        SSD1306_ShowStr(OLED_TEXT16_COLUMN_0, OLED_TEXT16_LINE_0, "no network", TEXT_SIZE_16);
        SSD1306_ShowStr(OLED_TEXT16_COLUMN_0, OLED_TEXT16_LINE_0, "          ", TEXT_SIZE_16);
    }
   
    memset_s((char *)oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    if (sprintf((char *)oledShowBuff, "sle_sate: %s", (g_sle_conn_flag == 1 ? "  ok" : "disc")) > 0) {
        SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT8_LINE_2, oledShowBuff, TEXT_SIZE_8);
    }
    memset_s((char *)oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    if (sprintf((char *)oledShowBuff, "power: %.1fV", (float)(systemValue.battery_voltage) / COEFFICIENT_1000) > 0) {
        SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT16_LINE_3, oledShowBuff, TEXT_SIZE_8);
    }
    /* 显示小车当前的状态 */
    memset_s((char *)oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    if (sprintf((char *)oledShowBuff, "track: %s", (systemValue.auto_track_flag == 1 ? " on" : "off")) > 0) {
        SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT8_LINE_4, oledShowBuff, TEXT_SIZE_8);
    }
    /* 显示小车当前的速度 */
    memset_s((char *)oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    ...
    SSD1306_ShowStr(OLED_TEXT8_COLUMN_0, OLED_TEXT8_LINE_5, oledShowBuff, TEXT_SIZE_8);
}
