                                    ...
                                    errcode_t status)
{
    ...
    if ((write_cb_para->length > 0) && write_cb_para->value)
    {
        ...
        // 检查接收到的数据中是否包含"LED_ON"命令
        if (osal_strstr((char *)write_cb_para->value, "LED_ON"))
        {
            // 若检测到"LED_ON"命令，将GPIO_13设置为低电平 LED点亮
            uapi_gpio_set_val(GPIO_13, GPIO_LEVEL_LOW);
        }
        // 检查接收到的数据中是否包含"LED_OFF"命令
        else if (osal_strstr((char *)write_cb_para->value, "LED_OFF"))
        {
            // 若检测到"LED_OFF"命令，将GPIO_13设置为高电平 LED熄灭
