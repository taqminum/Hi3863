...
void sle_client_init(void)
{
    ...
    printf("sle enable.\r\n");
    sle_client_seek_cbk_register();        // 注册设备扫描回调函数
    sle_client_connect_cbk_register();     // 注册连接管理回调函数
    sle_client_ssapc_cbk_register();       // 注册SSAP客户端协议栈回调函数
    if (enable_sle() != ERRCODE_SUCC) {    // 使能星闪协议栈
...
static void *sle_client_task(char *arg)
{
    ...
    sle_client_init();                     // 初始化星闪客户端
    while (1) {
        msg_data_t msg_data = {0};
        // 从消息队列读取数据（阻塞等待）
        int msg_ret = osal_msg_queue_read_copy(g_msg_queue, &msg_data, &g_msg_rev_size, OSAL_WAIT_FOREVER);
        if (msg_ret != OSAL_SUCCESS) {     // 读取消息队列失败
            printf("msg queue read copy fail.");
            if (msg_data.value != NULL) {
                osal_vfree(msg_data.value); // 释放内存空间
            }
        }
        if (msg_data.value != NULL) {      // 有效数据处理
            ssapc_write_param_t *sle_uart_send_param = &g_sle_send_param;
            sle_uart_send_param->data_len = msg_data.value_len; // 设置发送数据长度
            sle_uart_send_param->data = msg_data.value;         // 设置发送数据指针
            // 通过星闪协议向服务器发送数据（使用命令方式，无响应）
...
static void sle_client_entry(void)
{
    ...
    // 创建星闪客户端任务线程
    task_handle = osal_kthread_create((osal_kthread_handler)sle_client_task, 0, "sle_client_task", SLE_SERVER_STACK_SIZE);
    if (task_handle != NULL) {
        osal_kthread_set_priority(task_handle, SLE_SERVER_TASK_PRIO); // 设置任务优先级
        osal_kfree(task_handle);              // 释放任务句柄内存（任务已调度运行）
    }
