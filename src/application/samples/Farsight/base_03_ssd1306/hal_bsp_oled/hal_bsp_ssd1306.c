...
/**
 * @brief SSD1306 I2C数据发送函数
 * @note 该函数用于通过I2C总线向SSD1306 OLED显示屏发送数据或命令
 *       是底层硬件驱动与SSD1306显示控制器之间的桥梁
 * 
 * @param buff 待发送数据缓冲区指针，包含控制字节和实际数据
 *             - 第一个字节通常为控制字节（0x00表示命令，0x40表示数据）
 *             - 后续字节为实际的命令或显示数据
 * @param size 待发送数据的总长度（字节数），包括控制字节
 * 
 * @return uint32_t 错误代码
 *         - 0: 发送成功
 *         - 非0: 发送失败，具体错误码参考I2C驱动文档
 */
static uint32_t ssd1306_send_data(uint8_t *buff, size_t size)
{
    // SSD1306的I2C设备地址（通常为0x3C或0x3D）
    uint16_t dev_addr = SSD1306_I2C_ADDR;
    
    // 初始化I2C数据传输结构体
    i2c_data_t data = {0};
    data.send_buf = buff;  // 设置发送缓冲区指针
    data.send_len = size;  // 设置发送数据长度
    
    // 调用I2C主设备写操作API
    // 参数：I2C控制器索引、设备地址、数据传输结构体
    uint32_t ret = uapi_i2c_master_write(SSD1306_I2C_IDX, dev_addr, &data);
    
    // 检查I2C传输结果
    if (ret != 0) {
        // 打印错误信息，帮助调试
        // data.send_buf[1]通常是第一个实际数据字节（跳过控制字节）
...
/**
 * @brief 在SSD1306 OLED屏幕上显示字符串
 * @note 该函数支持两种字体大小：6x8和8x16像素
 *       使用预存储的字模数据进行字符显示
 * 
 * @param x 起始列坐标（0-127）
 * @param y 起始页坐标（0-7，每页8行像素）
 * @param ch 要显示的字符串（以null结尾）
 * @param text_size 字体大小
 *                 - TEXT_SIZE_8: 6x8像素字体
 *                 - TEXT_SIZE_16: 8x16像素字体
 */
void ssd1306_show_str(uint8_t x, uint8_t y, char *ch, uint8_t text_size)
{
    unsigned char c = 0;        // 字符转换后的字模索引
    unsigned char i = 0;        // 循环计数器（列数据索引）
    unsigned char j = 0;        // 循环计数器（字符串字符索引）
    uint8_t number_of_letter_lattice = 6;  // 6x8字体的列数（6列）
    uint8_t pos_x = x;          // 当前显示列位置
    uint8_t pos_y = y;          // 当前显示页位置

    // 根据字体大小选择不同的显示逻辑
    switch (text_size) {
        case TEXT_SIZE_8:  // 6x8像素小字体显示模式
            // 遍历字符串直到结束符
            while (ch[j] != '\0') {
                // 将ASCII字符转换为字模索引（从空格' '开始计算）
                c = ch[j] - ' ';
                ...
                for (i = 0; i < number_of_letter_lattice; i++) {
                    // 从F6X8字模数组中获取对应字符的列数据并写入
                    ssd1306_wite_data(F6X8[c][i]);
                }
                
                pos_x += TEXT_SIZE_8_WIDTH_6;  // 列位置右移6列（一个字符宽度）
                j++;                           // 处理下一个字符
            }
            break;

        case TEXT_SIZE_16:  // 8x16像素大字体显示模式
            // 调整页坐标（8x16字体占2页高度）
            pos_y *= OFFSET_2;
            ...
            while (ch[j] != '\0') {
                ...
                for (i = 0; i < TEXT_SIZE_16_WIDTH_8; i++) {
                    // F8X16字模数组结构：每个字符16字节，前8字节为上半部分
                ...
                pos_x += TEXT_SIZE_16_WIDTH_8;  // 列位置右移8列（一个字符宽度）
                j++;                            // 处理下一个字符
            }
            break;
