...
{
    // 创建发送数据缓冲区，仅包含1个字节的命令数据
    uint8_t buffer[] = {byte};
    
    // 初始化I2C数据传输结构体
    i2c_data_t i2cData = {0};          // 清零初始化
    i2cData.send_buf = buffer;         // 设置发送缓冲区指针
    i2cData.send_len = sizeof(buffer); // 设置发送数据长度（1字节）
    
    // 调用I2C主设备写操作API
    // 参数说明：
    //   SHT20_I2C_IDX:  使用的I2C控制器索引（如0,1,2...）
    //   SHT20_I2C_ADDR: SHT20的I2C设备地址
    //   &i2cData:       I2C数据传输结构体指针
