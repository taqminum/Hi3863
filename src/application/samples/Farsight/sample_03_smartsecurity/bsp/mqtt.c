...
#ifdef IOT
char *g_username = "SmartSecurity_ws63";
char *g_password = "fe4707cd02c595bcfa5074363db340154ea061685694145614a1cfe79a2f0616";
#endif
...
int messageArrived(void *context, char *topic_name, int topic_len, MQTTClient_message *message)
{
    ...
    // 进行传感器控制
    if (strstr((char *)message->payload, "buzzer") != NULL) {
        if (strstr((char *)message->payload, "ON") != NULL)
            set_buzzer(1);
        else if (strstr((char *)message->payload, "OFF") != NULL)
            set_buzzer(0);
    } else if (strstr((char *)message->payload, "autoMode") != NULL) {
        if (strstr((char *)message->payload, "ON") != NULL)
...
errcode_t mqtt_connect(void)
{
    ...
    while (1) {
        ...
        sprintf(g_send_buffer, MQTT_DATA_SEND, sys_msg_data.is_Body ? "ON" : "OFF",
                (sys_msg_data.buzzerStatus) ? "ON" : "OFF", sys_msg_data.smartControl_flag ? "ON" : "OFF");
