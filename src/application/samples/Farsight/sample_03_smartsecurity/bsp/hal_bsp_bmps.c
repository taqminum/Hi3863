...
void show_page(void)
{
    ...
    if (sys_msg_data.buzzerStatus != 0) {
        // 蜂鸣器状态 开启
        SSD1306_DrawBMP(png_right.left, png_right.top, png_right.width, png_right.hight,
                        bmp_32X32_BaoJing); // 显示 报警
        SSD1306_ShowStr(text_right.left, text_right.top, "Yes", TEXT_SIZE_16);
    } else {
        // 关闭
        SSD1306_DrawBMP(png_right.left, png_right.top, png_right.width, png_right.hight,
                        bmp_32X32_No_BaoJing); // 显示 不报警状态
        SSD1306_ShowStr(text_right.left, text_right.top, "No ", TEXT_SIZE_16);
    }

    if (sys_msg_data.is_Body != 0) {
        SSD1306_DrawBMP(png_left.left, png_left.top, png_left.width, png_left.hight, bmp_32X32_Body); // 显示 有人
        SSD1306_ShowStr(text_left.left, text_left.top, "Yes", TEXT_SIZE_16);
    } else {
        SSD1306_DrawBMP(png_left.left, png_left.top, png_left.width, png_left.hight, bmp_32X32_No_Body); // 显示 无人
