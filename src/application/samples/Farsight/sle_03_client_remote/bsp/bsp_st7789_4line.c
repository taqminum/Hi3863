...
double car_power;
uint8_t car_speed, car_conn;
char sle_recv[50];
uint8_t sle_recv_flag;
// SPI
...
void st7789_spi_master_init_config(void)
{
    spi_attr_t config = {0};
...
uint32_t ST7789_SPI_Transmit(spi_bus_t bus, uint8_t *txdata, uint32_t data_len, uint32_t timeout)
{
...
inline void ST7789_WR_REG(uint8_t REG)
{
    ...
    ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, &REG, 1, 0xffffffff); // 不读取从机返回数据
...
{
    // 写入数据
    ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, &DATA, 1, 0xffffffff); // 不读取从机返回数据
}
/*
**********************************************************************
* @fun     :ST7789_WriteReg
...
inline void ST7789_WriteRAM(uint16_t DAT)
{
    ...
    // 写入数据
    ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferD, 2, 0xffffffff); // 不读取从机返回数据
}
/*
**********************************************************************
...
void ST7789_SetCursor(uint16_t Xpos, uint16_t Ypos)
{
    ...
    // 传输数据
    ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferX, 2, 0xffffffff);

    // 设置坐标
    ST7789_WR_REG(ST7789dev.setycmd);

    // 传输数据
    ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferY, 2, 0xffffffff);
}
/*
**********************************************************************
* @fun     :ST7789_SetArea
...
void ST7789_SetArea(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    if (x0 != old_x0 || x1 != old_x1) {
        ...
        // 传输数据
        ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, arguments, 4, 0xffffffff);
...
void ST7789_Clear(uint16_t color)
{
    ...
    for (index = 0; index < totalpoint; index++) {
        ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferD, 2, 0xffffffff);
    }
}
/*
**********************************************************************
* @fun     :ST7789_HW_DrawPoint
* @brief   :uGUI函数调用，画点函数
* @param   :x,y:坐标，color:此点的颜色
* @return  :None
**********************************************************************
*/
void ST7789_HW_DrawPoint(uint16_t x, uint16_t y, uint16_t color)
{
...
/*
**********************************************************************
* @fun     :ST7789_HW_FillFrame
* @brief   :在指定区域内填充单个颜色
* @param   :(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1),color:要填充的颜色
* @return  :None
**********************************************************************
*/
void ST7789_HW_FillFrame(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color)
{
    uint16_t i = 0, j = 0;
    uint16_t xlen = 0;
    xlen = ex - sx + 1;
    ...
    for (i = sy; i <= ey; i++) {
        ...
        ST7789_WriteRAM_Prepare(); // 开始写入GRAM
        for (j = 0; j < xlen; j++) {
            ST7789_SPI_Transmit(SPI_MASTER_BUS_ID, TempBuffer, 2, 0xffffffff); // 点设置颜色
        }
    }
}
/*
**********************************************************************
* @fun     :ST7789_HW_DrawLine
* @brief   :画线
* @param   :_usX1,_usY1:起点坐标,_usX2,_usY2:终点坐标,_usColor:要填充的颜色
* @return  :None
**********************************************************************
*/
void ST7789_HW_DrawLine(uint16_t _usX1, uint16_t _usY1, uint16_t _usX2, uint16_t _usY2, uint16_t _usColor)
{
    int32_t dx, dy;
    int32_t tx, ty;
    int32_t inc1, inc2;
    ...
    ST7789_HW_DrawPoint(_usX1, _usY1, _usColor);
    /* 如果两点重合，结束后面的动作。*/
    ...
    while (x != _usX2) /* 循环画点 */
    {
        ...
        if (iTag) {
            ST7789_HW_DrawPoint(y, x, _usColor);
        } else {
            ST7789_HW_DrawPoint(x, y, _usColor);
        }
...
**********************************************************************
*/
void ST7789_LCD_DrawRect(uint16_t _usX, uint16_t _usY, uint16_t _usHeight, uint16_t _usWidth, uint16_t _usColor)
{
    ST7789_HW_DrawLine(_usX, _usY, _usX + _usWidth - 1, _usY, _usColor);                                 /* 顶 */
    ST7789_HW_DrawLine(_usX, _usY + _usHeight - 1, _usX + _usWidth - 1, _usY + _usHeight - 1, _usColor); /* 底 */

    ST7789_HW_DrawLine(_usX, _usY, _usX, _usY + _usHeight - 1, _usColor);                           /* 左 */
    ST7789_HW_DrawLine(_usX + _usWidth - 1, _usY, _usX + _usWidth - 1, _usY + _usHeight, _usColor); /* 右 */
}
/*
**********************************************************************
* @fun     :ST7789_LCD_DrawCircle
* @brief   :绘制一个圆，笔宽为1个像素
* @param   :
*			_usX,_usY  : 圆心的坐标
*			_usRadius  : 圆的半径
...
**********************************************************************
*/
void ST7789_LCD_DrawCircle(uint16_t _usX, uint16_t _usY, uint16_t _usRadius, uint16_t _usColor)
{
    ...
    while (CurX <= CurY) {
        ST7789_HW_DrawPoint(_usX + CurX, _usY + CurY, _usColor);
        ST7789_HW_DrawPoint(_usX + CurX, _usY - CurY, _usColor);
        ST7789_HW_DrawPoint(_usX - CurX, _usY + CurY, _usColor);
        ST7789_HW_DrawPoint(_usX - CurX, _usY - CurY, _usColor);
        ST7789_HW_DrawPoint(_usX + CurY, _usY + CurX, _usColor);
        ST7789_HW_DrawPoint(_usX + CurY, _usY - CurX, _usColor);
        ST7789_HW_DrawPoint(_usX - CurY, _usY + CurX, _usColor);
        ST7789_HW_DrawPoint(_usX - CurY, _usY - CurX, _usColor);
    ...
    for (uint8_t y = 0; y < 32; y++) {
        for (uint8_t x = 0; x < 24 / 8; x++) { // 假设每个字节表示8个像素点
            ...
            for (uint8_t bit = 0; bit < 8; bit++) {
                ...
                if (pixel) {
                    ST7789_HW_DrawPoint(x_start + x * 8 + bit, y_start + y, color);
                }
            }
        }
    }
    ...
    for (y = 0; y < 32; y++) {
        for (uint8_t byte_index = 0; byte_index < 1; byte_index++) { // 每行两个字节
            ...
            for (uint8_t bit = 0; bit < 8; bit++) {                  // 每个字节包含8个像素
                ST7789_HW_DrawPoint(x_start + bit, y_start + y, bgcolor);
                uint8_t pixel = (byte >> (7 - bit)) & 0x01; // 从高位到低位检查每个位
                if (pixel) {
                    // 注意修正x坐标的计算，确保正确绘制
                    ST7789_HW_DrawPoint(x_start + byte_index + bit, y_start + y, color);
                }
            }
        }
    }
...
void remote_display(void)
{
    ...
    if (sprintf(oledShowBuff, "ConTrol Mode: %s", ((select_mode > 0) ? " IR" : "SLE")) > 0) {
        DisplayString_chat(0, 0, LGRAY, oledShowBuff);
        memset_s(oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    }
    if (select_mode) {
        HW_DisplayImage(230, 0, 50, 50, gImage_ir);
    } else {
        HW_DisplayImage(230, 0, 50, 50, gImage_sle);
    }
    if (sprintf(oledShowBuff, "SLE Conn: %s", ((car_conn > 0) ? " OK" : "OFF")) > 0) {
        DisplayString_chat(0, 32, LGRAY, oledShowBuff);
        memset_s(oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    }
    if (sprintf(oledShowBuff, "Car Power: %.1f V", car_power) > 0) {
        DisplayString_chat(0, 64, LGRAY, oledShowBuff);
        memset_s(oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
    }
    ...
    DisplayString_chat(0, 96, LGRAY, oledShowBuff);
    memset_s(oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
}

void car_state_Parse(char *str)
{
    char *token = str + 1; // 跳过开头的 '{'

    // 解析 CAR_POWER
    token = strchr(token, ':') + 1;
    car_power = atof(token);
    token = strchr(token, ',');
