...
uint16_t Motor_PWM[3] = {35, 50, 65};
uint16_t Motor_Track_PWM[3] = {20, 27, 30};
...
extern uint32_t ALL_FLAG;
extern uint16_t ir, als, ps; // 人体红外传感器 接近传感器 光照强度传感器
...
void oled_show(void)
{
    if (ALL_FLAG) {
        ...
        memset_s((char *)oledShowBuff, sizeof(oledShowBuff), 0, sizeof(oledShowBuff));
        if (sprintf((char *)oledShowBuff, "power: %.1fV", (float)(systemValue.battery_voltage) / COEFFICIENT_1000) >
            0) {
