...
const char *local_name = "ServerNode";
uint8_t adv_handle = SLE_ADV_HANDLE_DEFAULT;
unsigned char local_addr[SLE_ADDR_LEN] = {0x03, 0x01, 0x01, 0x01, 0x01, 0x02}; // 本地设备地址
...
uint8_t connected_flag = 0; // 连接状态标志
...
{
    errcode_t ret;                     // 函数返回值，用于记录注册操作的状态
    ssaps_callbacks_t ssaps_cbk = {0}; // 回调函数结构体，初始化为全0

    // ==================== 设置所有回调函数指针 ====================
    // 以下回调函数对应GATT服务的各个生命周期事件和客户端操作事件
    ...
    // 描述符添加完成回调：当添加描述符完成后触发
    ssaps_cbk.add_descriptor_cb = ssaps_add_descriptor_cbk;

    // 服务启动完成回调：当启动服务完成后触发
    ssaps_cbk.start_service_cb = ssaps_start_service_cbk;

    // 服务删除完成回调：当删除所有服务完成后触发
    ssaps_cbk.delete_all_service_cb = ssaps_delete_all_service_cbk;

    // MTU改变回调：当连接的最大传输单元（MTU）发生变化时触发
    // MTU协商是连接建立后的重要步骤，影响单次传输的数据量大小
    ssaps_cbk.mtu_changed_cb = ssaps_mtu_changed_cbk;

    // 读取请求回调：当客户端请求读取特征值时触发
    // 应用程序需要在这个回调中提供特征值的当前数据
    ssaps_cbk.read_request_cb = ssaps_read_request_cbk;

    // 写入请求回调：当客户端请求写入特征值时触发
    // 应用程序需要在这个回调中处理写入的数据并更新特征值
    ssaps_cbk.write_request_cb = ssaps_write_request_cbk;

    // ==================== 注册回调函数到协议栈 ====================
    // 将设置好的回调函数结构体注册到星闪协议栈中
    // 协议栈会在相应事件发生时调用这些回调函数
    ...
    // ==================== 注册成功 ====================
    return ERRCODE_SLE_SUCCESS; // 返回成功错误码
}
...
static void *sle_terminal_task(const char *arg)
{
    ...
    while (1) {
        if (connected_flag == 1) {
            char data[] = "From ServerNode.";
            ssaps_ntf_ind_t param = {0};
            param.handle = g_property_handle;
            param.type = SSAP_PROPERTY_TYPE_VALUE;
            param.value_len = strlen(data);
