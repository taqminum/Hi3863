...
char host_ip[MAX_BUFF] = {0};   // 主机IP
char car_name[MAX_BUFF] = {0}; // 小车设备ID
...
int NFC_configwifi(uint8_t *nfc_data, uint8_t len)
{
    ...
    cJSON *carname = cJSON_GetObjectItem(root, "carname");
    if (root != NULL && ssid != NULL && password != NULL) {
        ...
        memcpy(host_ip, ip->valuestring, strlen(ip->valuestring));
        memcpy(car_name, carname->valuestring, strlen(carname->valuestring));

        printf("wifi_passwd = %s, wifi_passwd = %s, host_ip = %s, car_name = %s", wifi_passwd, wifi_passwd, host_ip, car_name);
